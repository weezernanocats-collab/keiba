...
async function main() {
  ...
  for (const race of filtered) {
    try {
      ...
      if (!shoshan || !shoshan.candidates || shoshan.candidates.length === 0) continue;
...
function buildMailHtml(
  ...
  return `
    <div style="max-width:600px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#1f2937;">
      ...
      <p style="font-size:12px;color:#6b7280;margin-bottom:12px;">${date} 発走前再生成 — しょーさん理論該当馬</p>
      ${warningHtml}
