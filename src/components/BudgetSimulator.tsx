...
export default function BudgetSimulator({ bets, riskLevel }: BudgetSimulatorProps) {
  ...
  const allocations = getAllocations(filteredBets, riskLevel, budget, allocMode === 'full');
  ...
  return (
    <div className="bg-card-bg border border-card-border rounded-xl p-6">
      ...
      <p className="text-sm text-muted mb-4">
        投資金額を入力すると、AIの戦略（リスク: {riskLevel === 'low' ? '低' : riskLevel === 'medium' ? '中' : '高'}）に基づいて自動配分します。
      </p>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="flex items-center gap-1">
          ...
          {(['full', 'kelly'] as const).map(mode => (
            ...
            >
              {mode === 'full' ? 'フル配分' : 'Kelly配分'}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-1">
          <span className="text-xs text-muted mr-1">対象:</span>
      ...
      {filterMode === 'ev_plus' && filteredBets.length === 0 && (
        <div className="text-center py-6 text-muted">
          <p>EV+馬券がありません</p>
          <p className="text-xs mt-1">全馬券モードに切り替えてください</p>
        </div>
      )}
                  ...
                  return (
                    <tr key={i} className="border-b dark:border-gray-800">
                      <td className="py-2">
                        ...
                        {isEvPlus && filterMode === 'all' && (
                          <span className="ml-1 px-1 py-0.5 text-[10px] bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded">
                            EV+
                          </span>
                        )}
                      </td>
...
function getAllocations(bets: Bet[], riskLevel: string, budget: number, normalize: boolean): Allocation[] {
  ...
  if (hasKelly) {
    ...
    // normalize=true(フル配分): 必ず予算100%を使い切る
    // normalize=false(Kelly配分): 合計が100%を超えない範囲でスケール
    const scale = normalize
      ? (totalStake > 0 ? 1 / totalStake : 1)
      : (totalStake > 1 ? 1 / totalStake : 1);
